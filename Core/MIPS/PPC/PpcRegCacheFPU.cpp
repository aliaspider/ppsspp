...
using namespace PpcJitConstants;
...
PPCReg PpcRegCacheFPU::R(int mipsReg) {
	if (mr[mipsReg].loc == ML_PPCREG) {
		...
		if (mipsReg < 32) {
			ERROR_LOG(JIT, "FReg %i not in PPC reg. compilerPC = %08x : %s", mipsReg, compilerPC_, currentDebugMIPS->disasm(compilerPC_, 0));
		} else if (mipsReg < 32 + 128) {
			ERROR_LOG(JIT, "VReg %i not in PPC reg. compilerPC = %08x : %s", mipsReg - 32, compilerPC_, currentDebugMIPS->disasm(compilerPC_, 0));
		} else {
			ERROR_LOG(JIT, "Tempreg %i not in PPC reg. compilerPC = %08x : %s", mipsReg - 128 - 32, compilerPC_, currentDebugMIPS->disasm(compilerPC_, 0));
		}
