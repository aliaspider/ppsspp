...
extern "C" GX2PixelShader GX2_fsTexCol, GX2_fsCol;
...
public:
	...
	~GX2VertexShaderModule() { free(shader_); }
...
public:
	...
	~GX2PixelShaderModule() { free(shader_); }
...
public:
	...
	~GX2GeometryShaderModule() { free(shader_); }
...
public:
	GX2TextureObject(const TextureDesc &desc) {
		...
		if (dataFormatToSwapSize(desc.format) == 2) {
			const u16 *src = (u16 *)desc.initData[0];
			...
			for (int i = 0; i < desc.height; i++) {
				...
				src += desc.width * DataFormatSizeInBytes(desc.format) / 2;
			}
		} else {
			const u32 *src = (u32 *)desc.initData[0];
			...
			for (int i = 0; i < desc.height; i++) {
				...
				src += desc.width * DataFormatSizeInBytes(desc.format) / 4;
			}
		}
#if 0
		DEBUG_STR(desc.tag.c_str());
...
void GX2DrawContext::BindVertexBuffers(int start, int count, Buffer **buffers, int *offsets) {
	if (!pipeline_) {
		ELOG("GX2DrawContext::BindVertexBuffers called without an active pipeline.");
	}
	if (pipeline_->inputLayout_->strides.size() > start + count) {
		ELOG("GX2DrawContext::BindVertexBuffers called invalid start + count.");
		return;
	}
...
void GX2DrawContext::Draw(int vertexCount, int offset) {
	if (!pipeline_) {
		ELOG("GX2DrawContext::Draw called without an active pipeline.");
		return;
	}
#if 0
	struct Vertex {
...
void GX2DrawContext::DrawIndexed(int indexCount, int offset) {
	if (!pipeline_) {
		ELOG("GX2DrawContext::DrawIndexed called without an active pipeline.");
		return;
	}
	if (!indexBuffer_) {
		ELOG("GX2DrawContext::DrawIndexed called without an active index buffer.");
		return;
	}
	if (!indexCount)
		return;
